...
(function () {
  ...
  function render(){
    ...
    // --- Companion (Morozane's lion; absent from sheets that have none) ---
    if(S.companion) grid.appendChild(buildCompanion());
    ...
    // --- Bushido / motivations ---
    // A Foundry export need not carry any of these: Morozane's has empty tenets
    // and no ninjō or giri recorded. Show the card only when there is something
    // in it, and never assume the keys exist — a missing one used to abort the
    // whole grid render and leave the sheet as a bare header.
    var B = S.bushido || {};
    var motRows = [["Paramount", B.paramount], ["Least significant", B.less],
                   ["Ninjō (desire)", S.ninjo], ["Giri (duty)", S.giri],
                   ["Register", B.register]];
    if(motRows.some(function(r){ return r[1]; })){
      var cMot = el("div","sh-card");
      cMot.appendChild(el("h2",null,"Bushidō &amp; Motivation"));
      var dl=el("div","deflist");
      motRows.forEach(function(r){
        if(r[1]) dl.appendChild(el("div","row","<dt>"+r[0]+"</dt><dd>"+r[1]+"</dd>"));
      });
      cMot.appendChild(dl);
  ...
  // An Animal Bond companion is an Adversary block, not a character: threat
  // ranks and a demeanour rather than social standing, and no trackers of its
  // own on this sheet. Rendered read-only — it is reference, not state.
  function buildCompanion(){
    var c=S.companion, card=el("div","sh-card comp-card");
    card.appendChild(el("h2",null,c.name));
    if(c.kind) card.appendChild(el("p","comp-kind",c.kind));
    var meta=[];
    if(c.threat) meta.push("Combat "+c.threat.combat+" · Intrigue "+c.threat.intrigue);
    if(c.demeanor) meta.push("Demeanour: "+c.demeanor);
    if(meta.length) card.appendChild(el("p","comp-meta",meta.join(" · ")));
    var r=el("div","comp-rings");
    ["air","earth","fire","water","void"].forEach(function(k){
      if(c.rings&&c.rings[k]!=null)
        r.appendChild(el("span","comp-ring","<b>"+c.rings[k]+"</b> "+k.charAt(0).toUpperCase()+k.slice(1)));
    });
    card.appendChild(r);
    var d=el("div","comp-derived");
    ["endurance","composure","focus","vigilance"].forEach(function(k){
      if(c.derived&&c.derived[k]!=null)
        d.appendChild(el("span","comp-stat","<b>"+c.derived[k]+"</b> "+k));
    });
    card.appendChild(d);
    if(c.skillGroups){
      var g=el("div","comp-derived");
      Object.keys(c.skillGroups).forEach(function(k){
        g.appendChild(el("span","comp-stat","<b>"+c.skillGroups[k]+"</b> "+k));
      });
      card.appendChild(g);
    }
    if(c.tnMods) card.appendChild(el("p","comp-meta","TN modifiers — "+c.tnMods));
    if(c.ability) card.appendChild(el("p","comp-ability",syms(c.ability)));
    if(c.note) card.appendChild(el("p","comp-note",c.note));
    return card;
  }
